//线性表的链式表示
// 双向循环链表
// 从头结点开始，后指针域依次指向下一个数据的存储地址，前指针域之前前一个数据的存储地址
//尾节点的后指针域指向头结点存储地址 构成环 头指针的前指针域指向尾节点数据的存储地址 
/*
...
参考 http://www.cnblogs.com/rookiefly/p/3452676.html
...
}DULNode, *LinkList;//链表 结构体变量  结构体指针 
...
{
    *L = (LinkList) malloc(sizeof(DULNode));// 分配头结点  一个元素 + 一个指针变量  的内存大小 
    if (*L == NULL) //分配失败
    {
        exit(OVERFLOW);
    }
    (*L)->next = *L;  //后继指针域指向它本身
    (*L)->prior = *L;  //前驱指针域指向它本身
}
...
{
    LinkList p = L->next;//p指向链表第二个元素
    while (p != L) //p未到表头时
    {
        if ( q->data == cur_e)//q节点的元素与目标 元素相等且q未到尾 
        {
            *pre_e = p->prior->data;// 
            return OK;
        }
...
{
    LinkList p = L->next->next;//p指向链表第二个元素 
    while (p != L) //未到表头时
    {   
        if (p->prior->data == cur_e)//节点元素与目标元素相等  且其后继为指向头 
        {
            *next_e = p->data;//当前节点的后继节点的元素为 目标元素的后继元素 
            return OK;
        }
        p = p->next;//指向后继  
    }
