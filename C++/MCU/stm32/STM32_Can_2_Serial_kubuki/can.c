...
// 	CAN_ITConfig(CAN1,CAN_IT_FMP0,ENABLE);//FIFO0消息挂号中断允许.	在，main函数开启	    
...
//printf("id:%d",RxMessage.StdId);
	...
	if(RxMessage.StdId == 0x02)// 左轮
	{
	  ptForkLift->s32EncoderCounter_l= RxMessage.Data[0]|(RxMessage.Data[1]<<8)|(RxMessage.Data[2]<<16)|(RxMessage.Data[3]<<24);
		ptForkLift->u16ADC_Carrent_l=RxMessage.Data[5]|(RxMessage.Data[6]<<8);
	  ptForkLift->u16CanComCount[0]++;	//CAN 接收 主控制器 状态 计数器	
		ptForkLift->u8Serial_Msg_Send_flag_Left=1;
  }
	
		if(RxMessage.StdId == 0x03)// 右轮
	{
	 	  ptForkLift->s32EncoderCounter_r= RxMessage.Data[0]|(RxMessage.Data[1]<<8)|(RxMessage.Data[2]<<16)|(RxMessage.Data[3]<<24);
		ptForkLift->u16ADC_Carrent_r=RxMessage.Data[5]|(RxMessage.Data[6]<<8);
	  //ptForkLift->u16CanComCount[0]++;	//CAN 接收 主控制器 状态 计数器	
		ptForkLift->u8Serial_Msg_Send_flag_Right=1;
  }
	
		if(RxMessage.StdId == 0x04)// 抬升电机
	{
	  ptForkLift->s32EncoderCounter_a= RxMessage.Data[0]|(RxMessage.Data[1]<<8)|(RxMessage.Data[2]<<16)|(RxMessage.Data[3]<<24);
		ptForkLift->u16ADC_Carrent_a=RxMessage.Data[5]|(RxMessage.Data[6]<<8);
	  //ptForkLift->u16CanComCount[0]++;	//CAN 接收 主控制器 状态 计数器	
		ptForkLift->u8Serial_Msg_Send_flag_Arm=1;
  }
