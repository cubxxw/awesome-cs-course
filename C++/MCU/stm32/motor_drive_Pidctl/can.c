...
//	}	
	...
	{
		if(ptForkLift->u8CarID==0)//前左
		{
			//ptForkLift->bDrection    = (RxMessage.Data[0]&0x01); // 0为前进  1为后退
			if (RxMessage.Data[0]&0x01)
			     ptForkLift->s16speedwant = -40*RxMessage.Data[1];//后退  放大倍数注意与 上位机设置一致
			else ptForkLift->s16speedwant = 40*RxMessage.Data[1]; //前进
		}
		else if(ptForkLift->u8CarID==1)//前右
		{
		...
		else if(ptForkLift->u8CarID==2)//后左
		{
		...
		else if(ptForkLift->u8CarID==3)//后左
		{
		...
		else if(ptForkLift->u8CarID==4)//前左
		{
		...
		else if(ptForkLift->u8CarID==5)//前右
		{
	...
	ptForkLift->Encoder_can_Tx_flag = 1;//设置can消息发送标志
	   
  ptForkLift->u16CanComCount[0]++;    // can 消息接收计数
	
	// ptForkLift->u16RunPIDControl = 1;   //wyw 添加 设置pid控制标识  在系统 1ms时钟中断内进行控制
