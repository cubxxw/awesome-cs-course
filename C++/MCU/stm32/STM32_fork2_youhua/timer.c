	...
	*/	
      if(ptForkLift->u16RunSendPeriodMotor == 50)// 50ms发送一次  移入 主程序 WHILE(1)中
		...
		//		ptForkLift->u16time_ms = 3001;
			...
			{					
				//包飞:发现编码器的计数值是实际的4倍
				ptForkLift->s16EncoderSpeed=(ENCODER_TIMER->CNT-15000) ;//采集编码器的速度
...
//			  printf("\r\n\tVOL_SET_Point\t");
//			  printf("%f\r\n",ptForkLift->s16speedwant);
  //    encoder_fliter = (ptForkLift->s16EncoderSpeed + encoder_fliter*7 ) / 8;
		//	printf("%d\r\n",(int)(encoder_fliter));
				
 // 		POSITION_PID(ptForkLift,(int)(encoder_fliter));
