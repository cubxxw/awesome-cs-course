...
//TIM_SetCompare2(TIM1,1800*10*2);           // 通道2 pwm 为 100% 脉冲 即为高电平
...
//printf("%d\r\n",ptCtrlCar->u16PWM);
  ...
  //SetPwmDir(ptCtrlCar);
 // if(BOARD_version==0) SetPwmDir(ptCtrlCar);        //左侧  老板子
	//else if(BOARD_version==1) SetPwmDir_new(ptCtrlCar);//右侧 新板子

	
	
	
	if(board_type==0)
		SetPwmDir(ptCtrlCar);        //左侧  老板子
	else
		SetPwmDir_new(ptCtrlCar);//右侧 新板子

/*
		if (ptForkLift->u8CarID==1)
		SetPwmDir_new(ptForkLift);//  左侧 最新焊的
	else
		SetPwmDir_new_inv(ptForkLift);//  右侧 原先反转的 换过来不知道什么原因 猛转
*/
}
