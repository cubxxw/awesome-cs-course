...
//tbs2:Ê±¼ä¶Î2µÄÊ±¼äµ¥Ôª.·¶Î§:1~8;    CAN_BS2_1tq ... CAN_BS2_8tq
    ...
    GPIO_Init(GPIOA, &GPIO_InitStructure);//³õÊ¼»¯IO
 	...
 	CAN_InitStructure.CAN_ABOM=DISABLE;						 //Èí¼ş×Ô¶¯ÀëÏß¹ÜÀí	 //
  	...
  	CAN_Init(CAN1, &CAN_InitStructure);            // ³õÊ¼»¯CAN1 
 	  ...
 	  CAN_FilterInitStructure.CAN_FilterNumber=0;	  //¹ıÂËÆ÷×é0 STM32ÆÕÍ¨ĞÍµÄĞ¾Æ¬CANÓĞ14×é¹ıÂËÆ÷ (»¥ÁªĞÍÓĞ28×é) Ã¿×é¹ıÂËÆ÷¿ÉÒÔÓĞ 1¡¢2¡¢4¸ö¹ıÂËÆ÷
   	CAN_FilterInitStructure.CAN_FilterMode=CAN_FilterMode_IdMask; 
		//ÆÁ±ÎÎ»Ä£Ê½  CAN_FilterMode_IdMask  CAN_FxR1ÖĞÄ³Ò»Î»Îª1µÄ»°£¬CAN_FxR0ÖĞÏàÓ¦Î»µÄÊı×Ö ±ØĞëÓëÊÕµ½µÄ canµÄ±êÊ¶·ûÒ»ÖÃ£¬canÏûÏ¢²ÅÄÜÍ¨¹ı¹ıÂË
		//±êÊ¶·ûÁĞ±íÄ£Ê½ CAN_FilterMode_IdList ÊÕµ½µÄcanµÄ±êÊ¶·û±ØĞëÓë CAN_FxR0 CAN_FxR1ÖĞµÄÒ»¸öÎÇºÏ²ÅÄÜÍ¨¹ı
		// 31-21Î» ¶ÔÓ¦ STDID 10-0Î»
		// 20-3 Î» ¶ÔÓ¦ EXID  0-17Î»
		// 2Î»     ¶ÔÓ¦ IDE
		// 1Î»     ¶ÔÓ¦ RTR   
		// 0Î»     Îª0  ²»¶ÔÓ¦
  	CAN_FilterInitStructure.CAN_FilterScale=CAN_FilterScale_32bit; //32Î»   32Î»¹ıÂËÆ÷ 
  	CAN_FilterInitStructure.CAN_FilterIdHigh=0x0020;// Ö¸¶¨21Î»Îª1ÆäÓà stdidÎª0   32Î»ID    ÊÕµ½µÄÓëÕâ¸ö ÔÚMASKÖĞÖ¸¶¨µÄÎ»ÉÏµÄÊı×ÖÒ»Ñù
  	CAN_FilterInitStructure.CAN_FilterIdLow=0x0000;//
  	CAN_FilterInitStructure.CAN_FilterMaskIdHigh=0xffe0;//32Î»MASK Ö¸¶¨Æ¥Åä 31-21Î» ¶ÔÓ¦ STDID 10-0Î»
  	CAN_FilterInitStructure.CAN_FilterMaskIdLow=0x0000;// 
  	CAN_FilterInitStructure.CAN_FilterFIFOAssignment=CAN_Filter_FIFO0;//¹ıÂËÆ÷×é0¹ØÁªµ½FIFO0
...
*/
///*
  int pid_t = ptForkLift->pid_time;
	if(RxMessage.Data[7]==0x00)//ÏûÏ¢ÀàĞÍÎª  6¸öµç»ú¿ØÖÆÖ¸Áî
	{
//////Ç°ÂÖ×ªËÙ
		if(ptForkLift->u8CarID==1)  //Ç°ÂÖ  v1=Vy+w*R
				...
				ptForkLift->s16speedwant = (float)((ptForkLift->s16y + ptForkLift->s16w * ptForkLift->s16dp_d*0.5)/(3.14*ptForkLift->s16dpl_d)*ptForkLift->u32dpl_econder*ptForkLift->u32dpl_econder_reduc/1000*pid_t); //1msÄÚµÄÂö³åÊı 
	...
	if(ptForkLift->u8CarID==2)  //×óÂÖ v1=-Vx+w*R
		{
				...
				if ((RxMessage.Data[6] & 0x20)) // ×Ö½Ú6 µÄbit5
				     ...
				     ptForkLift->s16x = -(float)(RxMessage.Data[2]/255.0*0.8);  //0~255 -> 0~0.8m/s
				//*/
				ptForkLift->s16speedwant = (float)((-ptForkLift->s16x + ptForkLift->s16w * ptForkLift->s16dp_d*0.5)/(3.14*ptForkLift->s16dpl_d)*ptForkLift->u32dpl_econder*ptForkLift->u32dpl_econder_reduc/1000*pid_t); //1msÄÚµÄÂö³åÊı 
		}
////////	ºóÂÖ					
...
////////ÉÏÏÂ²æ					
	if(ptForkLift->u8CarID==5)  //ÉÏÏÂ²æ
						{
							// ptForkLift->s16speedwant = (float)(ptForkLift->s16ud/(3.14*ptForkLift->s16ud_d)*ptForkLift->u32ud_econder*ptForkLift->u32ud_econder_reduc/1000*pid_t); //1msÄÚµÄÂö³åÊı
	...
	if(ptForkLift->u8CarID==6)  //Ç°ºó²æ
		{
				...
				if ((RxMessage.Data[6] & 0x80)) // ×Ö½Ú6 µÄbit5
					   ...
					   ptForkLift->s16fb = -(float)(RxMessage.Data[4]/255.0*0.32); //0~255 -> 0~0.32m/s
				//*/
				ptForkLift->s16speedwant = (float)(ptForkLift->s16fb/(3.14*ptForkLift->s16fb_d)*ptForkLift->u32fb_econder*ptForkLift->u32fb_econder_reduc/1000*pid_t); //1msÄÚµÄÂö³åÊı 
						 //ptForkLift->s16speedwant =0;
		}
				//*/
