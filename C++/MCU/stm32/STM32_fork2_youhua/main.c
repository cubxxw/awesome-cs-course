...
/************************************************
 ...
 2017/10/27  优化can滤波器设置  下位机只接受 上位机的can消息中断  下位机之间不接收 提高cpu效率
...
u8 debug_pid  =0;// 为1正常运行  为0 用于调试 pid
...
///*
	//四轮车 1ms控制周期  参数过小 震荡厉害  0.19  0.03  3  //  1ms  35———> 0.5m/s  18----->0.2m/s
	...
	if(ptForkLift->u8CarID==4)//左轮参数单独配置
	{
	 //四轮车 1ms控制周期  稳态时震荡稍大
  ptForkLift->pid_time=1.0;
  ptForkLift->s16speedwant = 0;   	
 ...
 //*/
//四轮车 叉车 10ms控制周期    0.5m/s   350脉冲   0.2m/s   140未除以4
/*
...
//四轮车 叉车 25ms控制周期    0.5m/s   875脉冲  0.2m/s  350脉冲  未除以4  一圈27500
/* 
	  ptForkLift->pid_time=25;  
...
// ptForkLift->s16speedwant = 171;   //  1ms  35———> 0.5m/s  18----->0.2m/s   10ms   140 ————> 0.2m/s  25ms  350  ————> 0.2m/s
	                                  // 未除以4  一圈脉冲27500
	                                  // 一圈4s的话  27500/4/1000=6.875  1ms 脉冲    10ms脉冲  68.75   25ms脉冲  171.875
