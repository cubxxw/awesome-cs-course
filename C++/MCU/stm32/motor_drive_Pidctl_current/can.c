...
//	}
	...
	if(RxMessage.StdId == 0x01)//firelay 发来的消息的 can_stdid  为 0x01 
	{
		...
		if(ptForkLift->u8CarID==0)//右侧轮
		{
       if((( RxMessage.Data[7] & 0x02) >> 1 ) ==1 ) //反转
            ptForkLift->s16speedwant = -((RxMessage.Data[3]<<8) | RxMessage.Data[2]); //1ms脉冲
			 else ptForkLift->s16speedwant =  ((RxMessage.Data[3]<<8) | RxMessage.Data[2]);
		}
		...
		if(ptForkLift->u8CarID==1)//左侧轮
		{
       if(( RxMessage.Data[7] & 0x01 ) == 1 )//反转 
            ptForkLift->s16speedwant = -((RxMessage.Data[1]<<8) | RxMessage.Data[0]); //1ms脉冲
			 else ptForkLift->s16speedwant =  ((RxMessage.Data[1]<<8) | RxMessage.Data[0]);
		}
