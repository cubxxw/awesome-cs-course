	...
	{			
     ...
     //*/	
		...
		if(debug_pid)//进行 pid测试时为0  正常运行改为1
		 {
			 	if((ptForkLift->u8Cmd_receive_lag == 1)&&(ptForkLift->u8SerialCmdFlag))// 接收到了正确的 命令  且串口 接收命令正常  更新速度设定值
					{
								...
								ptForkLift->s16speedwant  = ((ptForkLift->s16x - ptForkLift->s16w*disance_m/2.0)*3650);//左轮 201  减速比19.1 逆时针正转 正方向
								ptForkLift2->s16speedwant = ((-ptForkLift->s16x - ptForkLift->s16w*disance_m/2.0)*3650);//右轮 202  逆时针正转 反方向
					}
...
// kubuki 通信格式
   ...
   tempBuf[tempIndex] = chr;
	 tempIndex ++;
	 if (tempIndex>10)  tempIndex = 0;//过长
   if (update(chr))
   {
	  //kubuki 通信接口标识符 identifier
	  identifier = buffer[3];
