	...
	if (TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET)  //检查TIM3更新中断发生与否
		{
		  TIM_ClearITPendingBit(TIM3, TIM_IT_Update  );  //清除TIMx更新中断标志 
			...
			ptForkLift->u16RunRecvPeriodMotor++;//can 口接收状态更新 计数
			ptForkLift->u16RunSendPeriodMotor++;//串口上传信息 计时
			ptForkLift->u16RunPIDControl++;//pid控制器计时
			...
			/*进行CAN离线状态与在线状态的判断*/
			ptForkLift->u16CanComPeriod++; //CAN 201 接收计数  
			ptForkLift2->u16CanComPeriod++; //CAN 202 接收计数 
			...
			if(ptForkLift->u16CanComPeriod==100)
			{
				...
				ptForkLift->u16SerialComCount[1]=ptForkLift->u16SerialComCount[0];//串口接收 命令 计数 
			}		
			...
			{	
				...
				if(ptForkLift->u16SerialComCount[0] > ptForkLift->u16SerialComCount[1])
					ptForkLift->u8SerialCmdFlag=1;//串口接收 命令 正常
				else 
		...
		// 转速  均值
		/*
			ptForkLift->s32speed50ms += ptForkLift->speed_rpm;//转速和
			ptForkLift2->s32speed50ms += ptForkLift2->speed_rpm;//转速和		
      if(ptForkLift->u16RunSendPeriodMotor >= 50)// 50ms发送一次  移入 主程序 WHILE(1)中
			{
				  ptForkLift->u16RunSendPeriodMotor =0;
			//	ptForkLift->s16MeanSpeed = ptForkLift->s32speed50ms/50.0; //50ms内均值rpm   /60*0.1*3.14 为速度 m/s
...
// 通过 can 通信接口 下发 电流指令 给 电调 C620 控制无刷电机转动
