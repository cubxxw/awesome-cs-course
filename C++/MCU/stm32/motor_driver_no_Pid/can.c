...
//	}
	...
	if(RxMessage.StdId == 0x01)
	{
		...
		{
			ptForkLift->bDrection  = RxMessage.Data[1];    //方向
			ptForkLift->u16PWM     = 40*RxMessage.Data[0]; //pwm值
		}
		if(ptForkLift->u8CarID==1)
		{
			ptForkLift->bDrection  = RxMessage.Data[3];
			ptForkLift->u16PWM     = 40*RxMessage.Data[2];
		}
		
	}
	   
	ptForkLift->u16CanComCount[0]++;//can 中断 接收消息计数 
