...
```bash
git clone --depth 1 \
  https://ghproxy.com/https://github.com/preservim/nerdtree.git \
...
git clone --depth 1 \
  https://ghproxy.com/https://notabug.org/foo/foo.git \
...
```
git clone https://ghproxy.com/https://github.com/junegunn/vim
```
...
*我们直接在里面设置就好了：`vim ~/.vim/autoload/plug.vim `*
...
最后，启动 Vim 并提示 vim-plug 安装 ~/.vimrc 中列出的插件：*等待插件下载。*
...
```
git clone https://ghproxy.com/https://github.com/w0ng/vim-hybrid.git
```
